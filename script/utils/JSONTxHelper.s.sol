...
contract JSONTxHelper is Script {
    ...
    function _saveTx(address _from, address _to, uint256 _value, bytes memory _data, string memory _comment) internal {
    ...
    function _saveTxArrayToJson(string memory _action) internal {
        string memory json = "[";
        for (uint i = 0; i < transactions.length; i++) {
            ...
            json = string(abi.encodePacked(json, "{"));
            json = string(abi.encodePacked(json, '"from":"', vm.toString(transactions[i].from), '",'));
        ...
        string memory filename = string(abi.encodePacked("./script/admin-actions/output/", chainId, "/", _action, ".json"));
